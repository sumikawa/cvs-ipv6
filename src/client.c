     ...
     int do_gssapi;
{
    ...
    e = getaddrinfo(current_parsed_root->hostname, pbuf, &hints, &res0);
    if (e)
    {
	...
	if (trace)
	{
	    fprintf (stderr, " -> Connecting to %s(%s):%d\n",
		     current_parsed_root->hostname, aibuf, port_number);
	}
	...
	error (1, 0, "connect to %s(%s):%s failed: %s", 
	       current_parsed_root->hostname, aibuf, pbuf,
	...
	if (! connect_to_gserver (sock,
	    res0->ai_canonname ? res0->ai_canonname : current_parsed_root->hostname))
	{
	    error (0, 0,
